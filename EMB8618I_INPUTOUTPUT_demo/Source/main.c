...
//  DIN1
	...
	if (val)
	{
		...
		{ 
			DIValue |= 0x01;
			DOUT_Write(0x6);					// Turn ON LED (D01%D04) [Summer]
			printf("DI1 = 0\r\n");
		}
	}
	else
	{
		...
		{
			DIValue &= ~0x01;
			DOUT_Write(0xFF);					// Turn ON LED (D01%D04) [Summer]
			printf("DI1 = 1\r\n");
		}
	}
...
//  DIN3
	...
	if (val)
	{
		...
		{ 
			DIValue |= 0x04;
			DOUT_Write(0xED);				// Turn ON LED (D02%D05) [Summer]
			printf("DI3 = 0\r\n");
		}
	}
	else
	{
		...
		{
			DIValue &= ~0x04;
			DOUT_Write(0x3F);				// Turn ON LED (D02%D05) [Summer]
			printf("DI3 = 1, ON\r\n");
		}
	}
...
//  DIN5
	...
	if (val)
	{
		...
		{ 
			DIValue |= 0x10;
			DOUT_Write(0xDB);					// Turn ON LED (D03&D06) [Summer]
			printf("DI5 = 0\r\n");
		}
	}
	else
...
{	
	...
	//DOUT_Write(GPIOTest);
	DOUT_Write(0xFF);					// Turn OFF LED (D01-D06) [Summer]
	while (1)
	{		
		Delay_ms(20);							// 延时20ms
		DIN_Test();								// 输入测试
		...
		if ((cnt%50)==0)						// 每隔1秒
		{
			...
			if(count%4 == 0){
				//DOUT_Write(0x09);					// Turn ON LED (D01%D04) [Summer]	
				DOUT_Write(0xF6);					// Turn ON LED (D01%D04) [Summer]					
				count++;
			}
			else if(count%4 == 1){
				//DOUT_Write(0x12);				// Turn ON LED (D02%D05) [Summer]
				DOUT_Write(0xED);				// Turn ON LED (D02%D05) [Summer]
				count++;
			}
			else if(count%4 == 2){
				//DOUT_Write(0x24);					// Turn ON LED (D03&D06) [Summer]
				DOUT_Write(0xDB);					// Turn ON LED (D03&D06) [Summer]
				count++;
			}
			else if(count%4 == 3){
